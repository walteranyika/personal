...
public class HeartAttackActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getActionBar().setTitle(Html.fromHtml("<h5><font color='#ffffff'>Heart|Attack|FirstAid</font></h5>"));
		
		getActionBar().setIcon(R.drawable.woundicon);
		
info=(TextView) findViewById(R.id.tvHeartAttack);
Typeface font=Typeface.createFromAsset(getAssets(), "fonts/Aller.ttf");
info.setTypeface(font);
		
		String res = null;
		try {
			res = new TextFilesReader(this).readTxt("heartattack.txt");
		} catch (IOException e) {
	...
	{
		startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://www.youtube.com/watch?v=1qje-XlNowI")));
	}
