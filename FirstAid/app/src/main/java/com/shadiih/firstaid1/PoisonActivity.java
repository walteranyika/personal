...
public class PoisonActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getActionBar().setTitle(Html.fromHtml("<h5><font color='#ffffff'>Poison|First Aid</font></h5>"));
		
		getActionBar().setIcon(R.drawable.woundicon);
		
info=(TextView) findViewById(R.id.tvPoison);
Typeface font=Typeface.createFromAsset(getAssets(), "fonts/Aller.ttf");
info.setTypeface(font);
		
		String res = null;
		try {
			res = new TextFilesReader(this).readTxt("poison.txt");
		} catch (IOException e) {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.poison, menu);
		return true;
	}
	
	public void poison(View v)
	{
		startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://www.youtube.com/watch?v=L4o6D10AkMg")));
	}
	@Override
