...
public class BurnsActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getActionBar().setTitle(Html.fromHtml("<h5><font color='#ffffff'>Burns|First Aid</font></h5>"));
		
		getActionBar().setIcon(R.drawable.woundicon);
		
info=(TextView) findViewById(R.id.tvBurns);

Typeface font=Typeface.createFromAsset(getAssets(), "fonts/Aller.ttf");
info.setTypeface(font);
String res = null;
try {
	res = new TextFilesReader(this).readTxt("burn.txt");
} catch (IOException e) {
...
info.setText(Html.fromHtml(res));
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.burns, menu);
		return true;
	}
	
	public void burns(View v)
	{
		startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://www.youtube.com/watch?v=AxjjmaK2QAE")));
	}
